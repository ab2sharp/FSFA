#Generate Data for simulation and run sim (yes)
#Parameter Recovery
#setwd("G:/ResearchDocuments/Research/FunMatNorm/ParameterRecovery")
setwd("/u/ab2sharp/ResearchDocuments/Research/FunMatNorm/ParameterRecovery")
source("frankenclustAuto.R")
source("parrec_funs.R")
library(purrr)
load("params_fixxi.RData") #originally had params.R, now we use fixxi.
...
# I want to, choose a parameter set, and then, for that parameter set, 
# generate data, which I then cluster. After clustering, I want to subtract 
# the estimates from the true values, and then make inference on these. Better 
# to just return a list of all of the parameters for now. 
...
start =  Sys.time()
theboys = makeCluster( 110, type="FORK" )
registerDoParallel( theboys )
