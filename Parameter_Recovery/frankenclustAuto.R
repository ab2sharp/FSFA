...
trv2 = function(dt=NULL, par=NULL)
{
  ...
  trtm = apply( 
    r, 3, function(z, A=NULL, B=NULL){ sum(( z %*% tcrossprod(A,z) ) * B) }, 
    A=invS2, B=invS1
  ) #correct
...
ent = function(dat=NULL, gpar=NULL, map=FALSE)
{
  ...
  G = length(gpar$pi)
  ldens = sapply(
    1:G, FUN=function(g){ logden(datax=dat, gpar$dpar[[g]]) }
  ) #correct if logden is
  # n times g matrix
  # rows are observations, cols are groups
...
rzpar <- function(
  ...
  temp1 = eigen.sym(s1, k=qd[1])
  val$lam1  = temp1$vectors * 
  rep( 
    sqrt( temp1$values ), rep( nrow(temp1$vectors), length(temp1$values) )
  ) #correct
  val$xi1 =  diag(s1 - tcrossprod(val$lam1,val$lam1))
