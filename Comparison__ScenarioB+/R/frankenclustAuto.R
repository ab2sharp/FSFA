...
{
  e12 =  array(apply(r, c(2,3), crossprod, y=par$lam1 )^2, c(par$qd[1], dim(r)[2:3]))
  stop()
  f1 = array( apply(r, c(1,3), "%*%", y=par$lam2 ), c(par$qd[2], dim(r)[c(1,3)]))
  f12 = f1^2
  f22 = array( apply(f1, c(1,3), "%*%", y=par$lam1 )^2, c(par$q, dim(r)[3]))
  #print(dim(f22))
  t1 = apply(f22, c(3), function(z, v1, v2) { v1 %*% (z) %*% v2 }, v1=1/par$del1, v2=1/par$omg2 )
  #t21 = apply(e12, c(1,3), sum)
  #t22 = apply(f22, c(1,3), sum)
  t21 = array(apply(e12, 3, rowSums), c(par$q[1], dim(r)[3]))
  t22 = apply(f22, 3, rowSums)
  t2 = as.numeric( (1/par$del1) %*% (t21-t22) )/par$eta2
  
  
  #t31 = apply(f12, c(1,3), sum)
  t31 = array(apply(f12, 3, rowSums), c(par$q[2], dim(r)[3]))
  #t32 = apply(f22, c(2,3), sum)
  t32 = colSums(f22)
  t3 = as.numeric( (1/par$del2) %*% (t31-t32) )/par$eta1
  
  t43 = colSums(t21, dims=1) #t43 = apply(e12, 3, sum)
  t42 = colSums(t31, dims=1) #t42 = apply(f12, 3, sum)
...
logden <- function(datax=NULL, par=NULL) 
{
  ...
  logDet2 = -(par$pb[1]/2) * ( sum(log(par$omg2)) + (par$pb[2]-par$qd[2])*log(par$eta2) ) 
...
logdets <- function(datax=NULL, par=NULL) 
{
  ## datax is an array with dimension (p1 x p2 x n) 
  logc = -prod(par$pb)*log(2*pi)/2
  chols1 = determinant( par$lam1%*%t(par$lam1) + diag(par$xi1), logarithm = TRUE )$modulus
  
  logDet1 = -(par$pb[2]/2) * chols1
  logDet2 = -(par$pb[1]/2) * ( sum(log(par$omg2)) + (par$pb[2]-par$qd[2])*log(par$eta2) )  #not necessary
...
ent = function(dat=NULL, gpar=NULL, map=FALSE)
{
  ...
  return(val)
} #interpret: uncertainty (0 implies low uncertainty in the model)
...
#Random params
rgpar <- function(g=2, ddim=c(4,5), idim=NULL, nobs=NULL, hW=NULL, qs=NULL, ...) 
{
...
rzpar <- function(datx=NULL, zs=NULL, pb=NULL, qd=c(2,2), iter=10, auto=TRUE, 
                  thres=NULL, qinit=NULL)
{
  #
  # pb = (row, col) = (p, b)
  # qd = (latent gaussian dim, intrinsic basis dim) = (q, d)
  ...
  else
  {
    ...
    s2 = matrix(
      rowmeans( 
        apply( 
          r, 3, function(z, A=NULL){ t(z) %*% A %*% z }, A=diag(nrow=pb[1])
        )
      ), 
      nrow=pb[2] 
    )/pb[1]
...
rgzpar <- function(dt=NULL, g=2, ddim=c(4,5), idim=c(2,2), hW=NULL, qs=NULL, ...) 
{
  #q converted to a 2xG matrix
  nobs=dim(dt)[3]
  pb = dim(dt)[1:2]
...
cyc1 <- function(datax=NULL, par=NULL, zs=NULL, auto=TRUE, thres=NULL, hW=NULL) 
{
  ...
  ng = sum(zs)
  #mu
  par$mu = apply( sweep(datax, 3, as.numeric(zs)/ng, "*"), 
                  2, 
                  rowsums)
  ...
  if(auto)
  {
    ...
    #rd = which.max(relDiff(eigv)/eigv[-1]) 
...
cyc22 <- function(datax=NULL, par=NULL, zs=NULL) 
{
  ...
  invS1 = wbinv(par$lam1, par$xi1) #correct. Using solve gives same results. 
  invS1 = (invS1 + t(invS1))/2
  ...
  beta1 = t(par$lam1) %*% invS1 #correct
  vee   = diag( 
    par$pb[2], nrow=par$qd[1] ) - (par$pb[2])*(beta1 %*% par$lam1
  ) +  
    beta1 %*% W2 %*% t(beta1) 
